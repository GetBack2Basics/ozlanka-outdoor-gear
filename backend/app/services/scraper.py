...
async def _extract_product_from_page(page, url: str) -> ScrapedProduct:
    ...
    # Fallback: extract from page heading if no good name from JSON-LD
    if not name or name == title:
        for selector in ['h1.product-title', 'h1[itemprop="name"]', 'h1.product-name', '.product-title h1', 'main h1']:
            try:
                heading = page.locator(selector).first
                if await heading.count():
                    text = await heading.inner_text()
                    text = text.strip()
                    if text and text != title and "4WD Supacentre" not in text and "Home Page" not in text:
                        name = text
                        break
            except Exception:
                pass

    # Fallback: use og:title meta tag (strip site name suffix)
    # Also use if current name is generic (Home Page, etc.)
    if not name or name == title or "Home Page" in name:
        og_title = await _meta_content(page, 'meta[property="og:title"]')
        if og_title:
            # Remove " - 4WD Supacentre" suffix
            clean_title = og_title.replace(" - 4WD Supacentre", "").strip()
            if clean_title and clean_title != title and "Home Page" not in clean_title:
                name = clean_title
